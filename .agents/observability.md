...
- `src/__tests__/index.test.ts:350` — `describe('critical: core.setFailed is NEVER called (AC7)')`, covering happy path, no-files, parse error, API-error, and unexpected-exception cases. The empty-`api-key` (local-only) case lives separately at `src/__tests__/index.test.ts:2025`.
