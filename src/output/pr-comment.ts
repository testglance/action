...
export function renderBaseBranchSection(
  ...
  lines.push(`| Metric | ${baseBranch} | PR | Delta |`);
  lines.push('|--------|------|------|-------|');
  lines.push(
  ...
  const durPctSign = baseDelta.durationDeltaPercent >= 0 ? '+' : '';
  lines.push(
    `| Duration | ${formatDuration(baseDelta.durationPrev)} | ${formatDuration(baseDelta.durationCurr)} | ${durSign}${formatDuration(baseDelta.durationDelta)} (${durPctSign}${baseDelta.durationDeltaPercent.toFixed(0)}%) |`,
  );
